# Ciphered Lock (Randomizing Passcode)

An embedded "solve-for-it" cipher lock built on a Raspberry Pi Pico W. Instead of
entering a fixed code, the user solves a small puzzle at each step: the board shows two
operand symbols and an operation, and the user presses the symbol whose value is the
answer. The puzzle reshuffles after every unlock and after every lockout, so the input
is different every time.

Demonstrated with live LED, buzzer, and RGB-eye feedback, and built on a custom PCB
(currently on its second revision, Version B).

## How the cipher works

The interface is eight Greek letters (alpha ... theta), each with a value (alpha=1 ...
theta=8). For each step of the code:

1. Two operand letters light up on the 74HC595 LED bank.
2. The RGB eyes flash an operation color (purple = add, yellow = subtract, orange =
   multiply), then settle back to blue "entering" status.
...
The security concept: making the letter-to-value mapping secret (known only to the
...
|-------|------|------|-------|
| Armed (locked) | slow-breathe red | off | none |
...
- 74HC595 8-bit shift register driving 8 status/hint LEDs
- Two common-cathode RGB LEDs ("eyes"), PWM-driven
- Passive piezo buzzer
- Current-limiting resistors, decoupling capacitors
- Custom PCB (schematic and board files in `/docs`)

### Hardware revisions

- **Version B (current)** - upgraded board revision. Schematic, PCB layout, renders, and
  fabricated-board images are in `/docs` (`CLSCHEM_VER_B`, `CLPCB_VER_B`, `CL3D_VB_*`).
- **Version A (original)** - first fabricated revision, kept in `/docs` for reference
  (`CLSCHEM_VER_A`, `CLPCB_VER_A`, etc.).

## Software architecture

Firmware is written in MicroPython and is fully non-blocking: every peripheral advances
one step per loop pass off `time.ticks_ms()`, so the Wi-Fi keypad, the buzzer melodies,
and the eye animations all run concurrently without freezing each other.

| File | Role |
|------|------|
| `main.py` | Wires the peripherals and runs the non-blocking loop |
| `passcode.py` | Cipher brain: state machine + puzzle generation (`CONFIG` at top) |
...
reshuffles. In a finished product the code is conveyed through the hardware hint mapping,
not the console.

## Repository layout

- `*.py` - the MicroPython firmware
- `/docs` - schematics, PCB layouts, 3D renders, and fabricated-board images (Versions A and B)
...
Working demo on custom hardware (Version B). Enclosure and a physical locking actuator
are planned next.
